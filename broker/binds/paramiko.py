...
class Session:
    ...
    def __init__(self, **kwargs):  # noqa: PLR0912, PLR0915 - TODO: refactor?
        ...
        # Load host keys and mimic AutoAddPolicy behavior
        try:
            host_keys_path = Path("~/.ssh/known_hosts").expanduser().resolve()
            host_keys = paramiko.util.load_host_keys(host_keys_path)
            ...
            if not key_entry or key_entry.get(server_key.get_name()) != server_key:
                # Instead of printing, raise an error or handle according to a specific policy.
                # For now, raising an error indicating a host key issue.
                # A more robust solution might involve a custom HostKeys policy class.
                raise exceptions.ParamikoBindError(
                    f"Unknown or mismatched host key for {self.hostname}. "
                    f"Verify host key and add to '{host_keys_path}' manually if trusted."
                )
                # If automatic addition were desired (like AutoAddPolicy):
                # host_keys.add(self.hostname, server_key.get_name(), server_key)
                # host_keys.save(host_keys_path) # Requires write permissions

        except FileNotFoundError:
            # known_hosts file doesn't exist, proceed similar to first connection.
            # Consider logging this event if necessary.
            pass
        except Exception as e:
            # Catch other potential errors during host key handling
            raise exceptions.ParamikoBindError(f"Could not load/verify host keys: {e}") from e
