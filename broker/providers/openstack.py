...
from broker.settings import settings as default_settings
...
class OpenStack(Provider):
    ...
    def __init__(self, settings=None, **kwargs):
        ...
        self.settings = self._injected_settings or default_settings
        self._connection = None

    def _validate_settings(self, instance_name=None):
        """Handle injected settings for testing."""
        if self._injected_settings is not None:
            self.instance = instance_name
            return
        super()._validate_settings(instance_name)
    ...
    def _get_connection_with_app_credentials(self, auth_url):
        ...
        app_cred_id = getattr(self.settings.OPENSTACK, "application_credential_id", None)
        app_cred_secret = getattr(self.settings.OPENSTACK, "application_credential_secret", None)
        ...
        auth_config = {
            ...
            "identity_api_version": self.settings.OPENSTACK.identity_api_version,
        }

        for param in ["region_name", "interface"]:
            if hasattr(self.settings.OPENSTACK, param):
                auth_config[param] = getattr(self.settings.OPENSTACK, param)
    ...
    def _get_connection_with_password(self, auth_url):
        ...
        username = getattr(self.settings.OPENSTACK, "username", None)
        password = getattr(self.settings.OPENSTACK, "password", None)
        project_name = getattr(self.settings.OPENSTACK, "project_name", None)
        ...
        auth_config = {
            ...
            "user_domain_name": self.settings.OPENSTACK.user_domain_name,
            "project_domain_name": self.settings.OPENSTACK.project_domain_name,
            "identity_api_version": self.settings.OPENSTACK.identity_api_version,
        }
    ...
    def _get_connection(self):
        ...
        try:
            cloud_name = getattr(self.settings.OPENSTACK, "cloud", None)
            if cloud_name:
                return self._get_connection_with_cloud(cloud_name)

            auth_url = getattr(self.settings.OPENSTACK, "auth_url", None)
            if not auth_url:
    ...
    def provider_help(self, images=False, flavors=False, networks=False, templates=False, **kwargs):
        ...
        if images:
            ...
            templates = self.settings.OPENSTACK.get("templates", {})
            if templates:
    ...
    def _get_direct_params(self, kwargs):
        ...
        for param, default_key in default_mapping.items():
            if param not in params:
                params[param] = getattr(self.settings.OPENSTACK, default_key, None)
    ...
    def checkout(self, **kwargs):
        ...
        try:
            ...
            server_id = server.id
            timeout = self.settings.OPENSTACK.server_timeout
