...
def detect_and_reconfigure_for_aap25(base_url, settings_obj=None):
...
def resilient_job_wait(job, timeout=None, settings_obj=None):
    """Wait for a job to complete. Retry on errors."""
    _settings = settings_obj or settings
    timeout = timeout or _settings.ANSIBLETOWER.workflow_timeout
    completed = False
    while not completed:
        try:
...
def get_awxkit_and_uname(
    awxkit_config=None, root=None, url=None, token=None, uname=None, pword=None, settings_obj=None
):
    ...
    if not isinstance(awxkit_config, MockStub):  # skip if we're in a unit test
        ...
        detect_and_reconfigure_for_aap25(base_url=url, settings_obj=_settings)
