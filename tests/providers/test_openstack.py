...
def openstack_stub(api_stub):
    ...
    provider = OpenStack(settings=mock_settings)
...
def test_template_checkout(api_stub):
    ...
    mock_openstack_settings = MockStub({
        ...
        "server_timeout": 600,
        "get": lambda key, default=None: mock_openstack_settings.__dict__.get(key, default),
    })
...
def test_timeout_configuration(api_stub):
    ...
    assert provider.settings.OPENSTACK.server_timeout == 900
...
def test_connection_establishment_with_cloud():
    ...
    provider = OpenStack(settings=MockStub({"OPENSTACK": settings}))
